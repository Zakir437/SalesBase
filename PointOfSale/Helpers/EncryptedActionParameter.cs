            ...
            if (HttpContext.Current.Request.QueryString.Get("q") != null)
            {
                ...
                for (int i = 0; i < paramsArrs.Length; i++)
                {
                    ...
                    if (parametersType.ContainsKey(paramArr[0]))
                    {
                        ...
                        {
                            decryptedParameters.Add(paramArr[0], Convert.ToDouble(paramArr[1]));
                        }
                        else if (type == "Decimal")
                        {
                            decryptedParameters.Add(paramArr[0], Convert.ToDecimal(paramArr[1]));
                        }
                        else if (type == "DateTime")
                        {
                            if (!string.IsNullOrEmpty(paramArr[1]) && paramArr[1] != "null")
                            {
                                decryptedParameters.Add(paramArr[0], (Convert.ToDateTime(paramArr[1])));
                            }
                            else
                            {
                                decryptedParameters.Add(paramArr[0], null);
                            }
